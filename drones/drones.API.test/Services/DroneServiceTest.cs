...
namespace drones.API.test.Services
{
    ...
    internal class DroneServiceTest : BaseTest
    {
        ...
        public async Task CheckAvailableForLoading(HttpStatusCode statusCodeResult)
        {
        ...
        public async Task CheckBatteryCapacity(HttpStatusCode statusCodeResult)
        {
            string serialNumber = "1";

            if (TestContext.CurrentContext.Test.Name == "Check battery capacity OK")
            {
                // notthing to do
            }
            else if (TestContext.CurrentContext.Test.Name == "Check battery capacity whit empty serial number")
            {
                serialNumber = "";
            }
